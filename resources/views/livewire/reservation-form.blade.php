<div class="mx-auto" style="max-width: 36rem;">
    ...
    <form wire:submit="submit">
        <div class="mb-3">
            ...
            <select wire:model="scenario_id" id="scenario_id" class="form-select @error('scenario_id') is-invalid @enderror">
                <option value="">Selecciona un escenario</option>
                @foreach ($scenarios as $scenario)
                    <option value="{{ $scenario->id }}">{{ $scenario->name }} (capacidad: {{ $scenario->capacity }})</option>
                @endforeach
            </select>
            @error('scenario_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="mb-3">
            <label for="start_time" class="form-label">Fecha y hora de inicio</label>
            <input type="datetime-local" wire:model="start_time" id="start_time" class="form-control @error('start_time') is-invalid @enderror">
            @error('start_time') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="mb-3">
            <label for="end_time" class="form-label">Fecha y hora de fin</label>
            <input type="datetime-local" wire:model="end_time" id="end_time" class="form-control @error('end_time') is-invalid @enderror">
